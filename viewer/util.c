/*
 * $Id: util.c,v 1.100 2006/11/13 23:33:43 prussar Exp $
 *
    ...
    )
{
    ...
    RotDrawLine( box->topLeft.x, anchorY, box->topLeft.x + box->extent.x,
        anchorY );
}
...
static void WriteToMemo( void )
{
    ...
    for ( i = 0; i < DmNumRecords( InternalMemopadDB ); i++ ) {
        ...
        srcHandle = DmGetRecord( InternalMemopadDB, i );
        if ( srcHandle != NULL && 0 < MemHandleSize( srcHandle ) ) {
            ...
            DmReleaseRecord( InternalMemopadDB, i, false );
        }
    }
    ...
    )
{
    ...
    {
        recH = SafeDmResizeRecord( MemopadDB, *index, *offset + length + 1 );
        releaseRecord = false;
    }
...
{
    if ( ref == NULL || NULL == DmQueryRecord( ref, index ) )
        return NULL;

    return DmResizeRecord( ref, index, size );
}




void SafeSetRecyclable( UInt16 card, LocalID id )
{
    UInt16 attr;

    if ( id == NULL || ! Support40() )
        return;
...
/* Set a form title via a string resource */
void SetFormTitle( FormType* form, UInt16 id )
{
    MemHandle h;

    h = DmGetResource( 'tSTR', id );
...
void DBSetBackup( UInt16 cardNo, LocalID id )
{
    UInt16 attr;

    if ( errNone == DmDatabaseInfo( cardNo, id, NULL, &attr, NULL,
         NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL ) ) {
        if ( ! ( attr & dmHdrAttrBackup ) ) {
            attr |= dmHdrAttrBackup;
            DmSetDatabaseInfo( cardNo, id, NULL, &attr, NULL,
               NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL );
        }
    }
}
