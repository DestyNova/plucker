/*
 * $Id: font.c,v 1.54 2006/04/14 03:27:18 prussar Exp $
 *
     ...
     )
{
    ...
    for ( i = DmNumRecords( ref ) ; i < numFontCaches ; i++ ) {
        UInt16    index;
        MemHandle handle;

        index = i;
        handle = DmNewRecord( ref, &index, 1 );
    ...
    record = SafeDmResizeRecord( ref, cache, dataLen );
    ...
    if ( record != NULL ) {
        ...
        DmWrite( recordData, nameLen + volumeLabelLen + 2, path,
            pathLen + 1 );
        MemHandleUnlock( record );
        DmReleaseRecord( ref, cache, true );
    }
