/*
 * $Id: genericfile.c,v 1.36 2008/01/20 21:15:42 prussar Exp $
 *
...
Err OpenLastDocument( void )
{
    ...
    ErrTry { 
        ...
        handle = FindDocData( Prefs()->docName, ALL_ELEMENTS, &index );
...
/* Returns true if user opts to delete */
Boolean DoDeleteDocument(void)
{
    ...
    if ( selectedButton == SELECT_DELETE ) {
        ...
        ErrTry {
            ...
            DeleteMetaDocument( Prefs()->docName,
                                Prefs()->location );
            DeleteAnnotations( Prefs()->docName,
                                Prefs()->location );
            CloseDocList();
            ReleaseDocInfoList();
