/*
 * $Id: prefsform.c,v 1.81 2007/05/30 21:39:01 chrish Exp $
 *
...
#define NUM_OF_PREF_SECTIONS  12
...
static UInt16           handlerMap[ NUM_OF_PREF_SECTIONS ];
static Int16            totalSections;
static Int16            currentSection;
static Int16            numOfSections;
...
static UInt16           objectList[][ MAX_OBJECTS ] = {
    { frmPrefsGeneralMenuToolbarLabel, frmPrefsGeneralMenuToolbarPopup,
      ...
      frmPrefsGeneralScreenDepthLabel, frmPrefsGeneralScreenDepthPopup,
      frmPrefsGeneralScrollbarLabel, frmPrefsGeneralScrollbarPopup,
#ifdef SUPPORT_ANNOTATION
      frmPrefsGeneralAnnotationsInBookmarkList,
#endif
      frmPrefsGeneralAlwaysDeleteAnnotations,
      EOL },
    { frmPrefsLayoutUnderline, frmPrefsLayoutEnableSoftHyphens,
      frmPrefsLayoutPageControlsLink,
      frmPrefsLayoutAlignLabel, frmPrefsLayoutAlignPopup,
      frmPrefsLayoutLineSpacingLabel, frmPrefsLayoutLineSpacingPopup,
      frmPrefsLayoutParagraphSpacingLabel, frmPrefsLayoutParagraphSpacingPopup,
#ifdef HAVE_ROTATE
      frmPrefsLayoutRotateLabel, frmPrefsLayoutRotatePopup,
#endif
#ifdef HAVE_RTL
      frmPrefsLayoutRTL,
#endif
      frmPrefsLayoutEnableMargins,
      frmPrefsLayoutIndividual, frmPrefsLayoutAsDefault,
      EOL },
    { frmPrefsFontMainMode, frmPrefsFontLibraryMode,
      EOL },
    { frmPrefsColorStrikethrough, 
      frmPrefsColorForceDefaultColors,
      frmPrefsColorIndividual, frmPrefsColorAsDefault,
      EOL },
    { frmPrefsLibrarySyncPolicyLabel, frmPrefsLibrarySyncPolicyPopup,
      frmPrefsLibraryCategoryStyleLabel,
    ...
    { frmPrefsGestureGestures, frmPrefsGestureSelectAction, frmPrefsGestureUp,
      ...
      frmPrefsGestureDown, bmpGestureDown,
      EOL },
    { frmPrefsJogdialJogEnabled, frmPrefsJogdialSelectAction,
      EOL },
#ifdef SUPPORT_WORD_LOOKUP
    { frmPrefsLookupAlwaysActive,
      frmPrefsLookupActionPopup, frmPrefsLookupActionLabel,
      frmPrefsLookupIndividual, frmPrefsLookupAsDefault,
      EOL }
#endif
};
static HandlerType      handlerList[ NUM_OF_PREF_SECTIONS ] = {
    { PrefsGeneralPreferenceEvent, PrefsGeneralPalmEvent, strPrefsGeneral, strPrefsGeneralHelp },
    { PrefsLayoutPreferenceEvent, PrefsLayoutPalmEvent, strPrefsLayout, strPrefsLayoutHelp },
    { PrefsFontPreferenceEvent, PrefsFontPalmEvent, strPrefsFont, strPrefsFontHelp },
    { PrefsColorPreferenceEvent, PrefsColorPalmEvent, strPrefsColor, strPrefsColorHelp },
    { PrefsLibraryPreferenceEvent, PrefsLibraryPalmEvent, strPrefsLibrary, strPrefsLibraryHelp },
    { PrefsAutoscrollPreferenceEvent, PrefsAutoscrollPalmEvent,
      strPrefsAutoscroll, strPrefsAutoscrollHelp },
    { PrefsHardcopyPreferenceEvent, PrefsHardcopyPalmEvent,
      strPrefsHardcopy, strPrefsHardcopyHelp },
    { PrefsButtonPreferenceEvent, PrefsButtonPalmEvent, strPrefsButton, strPrefsButtonHelp },
    { PrefsControlPreferenceEvent, PrefsControlPalmEvent, strPrefsControl, strPrefsControlHelp },
    { PrefsGesturePreferenceEvent, PrefsGesturePalmEvent, strPrefsGesture, strPrefsGestureHelp },
    { PrefsJogdialPreferenceEvent, PrefsJogdialPalmEvent, strPrefsJogdial, strPrefsJogdialHelp },
    { PrefsLookupPreferenceEvent, PrefsLookupPalmEvent, strPrefsLookup, strPrefsLookupHelp },
};
...
static void PrefsFormInit( void )
{
    ...
    numOfSections  = NUM_OF_PREF_SECTIONS;
    ...
    for ( i = 0; i < numOfSections; i++ ) {
        if ( handlerList[ i ].PreferenceEvent( AVAILABLE ) ) {
            ...
            SysCopyStringResource( temp, handlerList[ i ].strID );
            temp += StrLen( temp ) + 1;
    ...
    PrefsShowSection( objectList, currentSection );
    ...
    )
{
    ...
    if ( isControl )
        CtlSetUsable( GetObjectPtr( id ), true );
}


/* Hide the object */
void HidePrefsFormObject
    (
    UInt16 id,          /* object id */
    ...
    )
{
    ...
    if ( isControl )
        CtlSetUsable( GetObjectPtr( id ), false );
}
    ...
    )
{
    ...
    PrefsHideSection( objectList, currentSection );

    /* Open the newly requested section */
    PrefsShowSection( objectList, setSection );
