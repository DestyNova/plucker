/*
 * $Id: anchor.c,v 1.64 2006/11/10 06:51:21 prussar Exp $
 *
...
static void AnchorCreate(const TextContext *tContext,
                ...
                const Int16 paragraphOffset, const Int16 anchorId,
                const AnchorMode mode ) ANCHOR_SECTION;
...
static void AnchorCreate
    ...
    const AnchorMode    mode        /* mode of anchor */
    )
{
    AnchorType*  anchor;
...
#ifdef SUPPORT_ANNOTATION
    if ( mode == ANCHORMODE_ANNOTATION )
        anchor->annotationID = GetCurrentAnnotationHeader()->id;
#endif
    ...
    )
{
    ...
    AnchorCreate( tContext, reference, NO_IMAGE, paragraphOffset, anchorId++,
        mode );
}
    ...
    )
{
    ...
    if ( anchorList != NULL ) {
        ...
        AnchorCreate( tContext, lastAnchor->reference, lastAnchor->image,
            ...
            lastAnchor->mode );
        AnchorSetByteOffset( tContext, lastAnchor->byteOffset );
    }
}
...
void DeleteUnusedAnchors( void )
{
    ...
    while ( anchor != NULL ) {
        ...
        next = ListNext( anchorList, anchor );
...
#ifdef SUPPORT_ANNOTATION
                else if ( lastAnchor->mode == ANCHORMODE_ANNOTATION ) {
                    HighlightRectangle( &( lastAnchor->bounds ), 0,
                        true, TEXT );
                }
#endif
            }
...
#ifdef SUPPORT_ANNOTATION
/* Return the mode of a visible anchor */
AnchorMode GetVisibleMode
    (
    const Int16 index   /* index of visible anchor */
    )
{
...
AnnotationIdentifier* GetVisibleAnnotationIdentifier
    (
    const Int16 index
    )
{
