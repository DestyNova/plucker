/*
 * $Id: control.c,v 1.147 2006/11/12 02:00:55 prussar Exp $
 *
...
static SelectedWordActionType  selectedWordAction;
#endif
    ...
    )
{
    ...
    if ( GetSelectedWord( &selectedWord.bounds, NULL, NULL, NULL ) != NULL ) {
...
void DoAddBookmark( void )
{
    ...
    uid               = meta->uid;
    paragraph         = meta->firstVisibleParagraph;
    offsetInParagraph = meta->characterPosition -
                            GetParagraphOffset( record,
                                GET_PARAGRAPH( record, paragraph ) );

    MemHandleUnlock( GetMetaRecord() );
    MemHandleUnlock( handle );

    DoAnnotate( NULL, ANNOTATION_BOOKMARK | ANNOTATION_HIDDEN,
        uid, paragraph, offsetInParagraph, offsetInParagraph, name );
}
    ...
    )
{
    ...
    delta = GetSequenceOffset() + ExtentY() -
                location * ( GetSequenceHeight() / 10 + 1 );
