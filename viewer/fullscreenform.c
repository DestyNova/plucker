/*
 * $Id: fullscreenform.c,v 1.52 2008/01/21 16:44:41 prussar Exp $
 *
...
static WinHandle     bufferWindow = NULL;
...
static void MoveImage( Coord x, Coord y, Boolean showControls ) 
                FULLSCREENFORM_SECTION;
...
static void ShiftImage( Coord dx, Coord dy ) FULLSCREENFORM_SECTION;
    ...
    )
{
    ...
    MoveImage( imageBounds.topLeft.x, imageBounds.topLeft.y, true );
...
static void ShiftImage( Coord dx, Coord dy )
{
    imageBounds.topLeft.x += dx;
    imageBounds.topLeft.y += dy;
    ...
    MoveImage( imageBounds.topLeft.x, imageBounds.topLeft.y, false );
}
    ...
    )
{
    ...
    switch ( FiveWayTranslate( event, ! fastEnough ) ) {
        case NOT_FIVEWAY:
            break;
        case NOT_ARROW:
            handled = true;
            break;
        case UP_ARROW:
            ShiftImage( 0, -screenBounds.extent.y / 5 );
            handled = true;
            break;
        case DOWN_ARROW:
            ShiftImage( 0, screenBounds.extent.y / 5 );
            ...
            break;
        case LEFT_ARROW:
            ShiftImage( -screenBounds.extent.x / 5, 0 );
            handled = true;
            break;
        case RIGHT_ARROW:
            ShiftImage( screenBounds.extent.x / 5, 0 );
