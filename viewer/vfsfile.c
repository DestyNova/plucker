/*
 * $Id: vfsfile.c,v 1.51 2008/01/20 22:21:33 prussar Exp $
 *
    ...
    )
{
    ...
    path = SafeMemPtrNew( StrLen( dir ) + StrLen( info->nameP ) + 3 );
    StrCopy( path, dir );
    StrCat( path, "/" );
    ...
    if ( err == errNone ) {
        ...
        if ( err == errNone && IsDocScan( enumerateWhat ) &&
            ...
            /* Try to retrieve the publication date from the document and, if
               found, use it instead of the file creation date */
            docInfo.cardNo      = 0;
    ...
    )
{
    ...
    err = OpenVFSDatabase( docInfo->volumeRef, path, &vfsDocument, &version,
            docInfo->name, &created, &docInfo->numRecords );
    if ( err != errNone )
        return err;
    ...
    /* THROWS */
{
    ...
    MakeMetaName( metaNewName, META_PREFIX, docInfo->location, newName );
    RenameMetaDocument( metaNewName, oldLocation, docInfo );
    MakeMetaName( metaNewName, ANN_PREFIX, docInfo->location, newName );
