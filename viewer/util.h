/*
 * $Id: util.h,v 1.78 2006/11/13 23:33:43 prussar Exp $
 *
...
{
    ANCHORMODE_NONE,
    ANCHORMODE_LINK    /* standard document anchors */
#ifdef SUPPORT_ANNOTATION
    ,ANCHORMODE_ANNOTATION
#endif
} AnchorMode;
...
/* This is a workaround for a suspected NVFS bug with resizing a record that
   has not been fetched. */
extern MemHandle SafeDmResizeRecord( DmOpenRef ref, UInt16 index, UInt32 size )
    UTIL_SECTION;

/* Marks a database as recyclable.  Does nothing if id == NULL. */
extern void SafeSetRecyclable( UInt16 card, LocalID id )
    UTIL_SECTION;

/* Set a form title via a string resource */
void SetFormTitle( FormType* form, UInt16 id ) UTIL_SECTION;

/* Set backup attribute for a database */
void DBSetBackup( UInt16 cardNo, LocalID id ) UTIL_SECTION;
