/*
 * $Id: table.c,v 1.68 2005/08/08 02:30:59 prussar Exp $
 *
    ...
    )
{
    ...
    if ( isLargeTable ) {
        ...
        tContext.cursorX = x; 
        tContext.cursorY = y;
        AnchorStart( &tContext, reference, offset, ANCHORMODE_LINK );
        if ( altImage )
    ...
    )
{
        ...
        if ( ptr[0] == '\0' ) {
            switch ( ptr[1] ) {
                ...
                case NEW_CELL:
                    ...
                    if ( acell->text )
                        GetTextMetrics( ptr, acell->len, &acell->width,
                                        &acell->height, false );
                    acell->splitHeight = saveHeight;
    ...
    *tableWidth = 1;
    ...
    /* Calc 'colspan' cell widths */
    for ( row = 0; row < table->rows; row++ ) {
        for ( col = 0; col < table->cols; col++ )
            if ( 1 < CELLS( row, col ).colspan ) {
                CELLS( row, col ).width = 0;
                for ( col2 = col; col2 < col + CELLS( row, col ).colspan;
                        col2++ )
                    CELLS( row, col ).width += col_base[col2];
            }
    }
        ...
        while ( tableAnchor != NULL ) {
            ...
            if ( anchorBounds.extent.x && anchorBounds.extent.y ) {
                ...
                AnchorStart( &tContext, tableAnchor->reference,
                             tableAnchor->offset, ANCHORMODE_LINK );
