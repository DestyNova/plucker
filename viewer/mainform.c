/*
 * $Id: mainform.c,v 1.158 2008/01/20 21:15:42 prussar Exp $
 *
...
static void GoSelectedWord ( SelectedWordActionType  action ) MAINFORM_SECTION;
#endif
...
#ifdef SUPPORT_PPI
            case mGoLookup:
                DoSelectTypeAction( SELECT_WORD_PPI );
                handled = true;
                break;
#endif
...
#ifdef HAVE_SILKSCREEN
    /* Correct for a T5 (probably also TX and LifeDrive) issue where
       FrmDrawForm() will sometimes change DIA state. */
    if ( correctDIA != GetDIAState() )
        SetDIAState( correctDIA );
#endif
...
#ifdef SUPPORT_WORD_LOOKUP
static void GoSelectedWord
    (
    SelectedWordActionType  action
    )
{
     Char*   word;
     UInt16  start;
     ...
     word = GetSelectedWord( NULL, &paragraph, &start, &stop );
     if ( word != NULL ) {
        switch ( action ) {
            ...
            case SELECT_WORD_ANNOTATE:
                DoAnnotate( NULL, 0, GetCurrentRecordId(), paragraph, start, 
                    stop, word );
                break;
#endif
    ...
    )
{
    ...
    switch ( event->eType ) {
        ...
        case frmGotoEvent:
            if ( event->data.frmGoto.matchCustom == GOTO_SEARCH ) {
        ...
        case pluckerSelectedWordEvent:
            GoSelectedWord( event->data.generic.datum[ 0 ] );
            handled = true;
            break;
#endif
        ...
        case frmUpdateEvent:
            ...
            if ( event->data.frmUpdate.updateCode == frmRedrawUpdateCode ||
                ...
                if ( mode == WRITEMODE_DRAW_CHAR ) {
                    ...
                    firstForm = FrmGetFirstForm();
                    if ( firstForm != NULL &&
                         FrmGetFormId( firstForm ) == frmFullscreen ) {
                        handled = true;
                        break;
                    }
                }
