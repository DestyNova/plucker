/*
 * $Id: libraryform.c,v 1.120 2007/02/15 15:16:43 prussar Exp $
 *
...
static void SetSortFunction( void ) LIBRARYFORM_SECTION;
...
static Int16 CompareDateTimeDesc( DocumentInfo* d1,
                DocumentInfo* d2 ) LIBRARYFORM_SECTION;
static Int16 CompareCategoryAsc( DocumentInfo* d1,
                DocumentInfo* d2 ) LIBRARYFORM_SECTION;
static Int16 CompareCategoryDesc( DocumentInfo* d1,
                DocumentInfo* d2 ) LIBRARYFORM_SECTION;
...
static void SetSortType( SortType sortType ) LIBRARYFORM_SECTION;
static void DoSort( void ) LIBRARYFORM_SECTION;
static void DoIndexUpdate( void ) LIBRARYFORM_SECTION;
...
static void DrawLayout( void )
{
    ...
    HiResAdjust( &scrollColumnWidth, sonyHiRes );
...
static void SetSortFunction( void )
{
    switch ( Prefs()->sortType ) {
        ...
        case SORT_CATEGORY:
            if ( Prefs()->sortOrder == SORTORDER_ASC )
                CompareItems = CompareCategoryAsc;
            else
                CompareItems = CompareCategoryDesc;
            break;
...
/* Compare categories ( ascending ), if match go by name ( ascending )  */
static Int16 CompareCategoryAsc
    (
    DocumentInfo* d1,   /* pointer to the record to sort */
    DocumentInfo* d2    /* pointer to the record to sort */
    )
{
    /*   0 if d1.categories = d2.categories */
    /* < 0 if d1.categories < d2.categories */
    /* > 0 if d1.categories > d2.categories */
    if ( d2->categories < d1->categories )
        return 1;
    else if ( d1->categories < d2->categories )
        return -1;
    else
        return CompareNameAsc( d1, d2 );
}
...
/* Compare categories ( descending ), if match go by name ( ascending )  */
static Int16 CompareCategoryDesc
    (
    DocumentInfo* d1,   /* pointer to the record to sort */
    DocumentInfo* d2    /* pointer to the record to sort */
    )
    ...
    /* THROWS */
{
    ...
    DeleteAnnotations( label, docInfo->location );
...
static void FormatDocument( UInt16 i, FormatType* format )
{
    ...
    if ( Support40() ) {
        created = TimUTCToTimeZone( docInfo->created,
    ...
    TimSecondsToDateTime( created, &dateTime );
...
static void CalculateColumnWidths( void )
{
    ...
    if ( Prefs()->column.type == SHOW ) {
        ...
        if ( bitmapH != NULL ) {
            ...
            if ( bitmap != NULL )
            {
                BmpGlueGetDimensions( bitmap, &typeColumnWidth, NULL, NULL );
                typeColumnWidth += 2;
    ...
    switch ( event->eType ) {
        ...
        case menuEvent:
            ...
            switch ( event->data.menu.itemID ) {
                ...
                case mSortDate:
                    SetSortType( SORT_DATE );
                    DoSort();
                    handled = true;
                    break;

                case mSortSize:
                    SetSortType( SORT_SIZE );
                    DoSort();
                    handled = true;
                    break;

                case mSortCategory:
                    SetSortType( SORT_CATEGORY );
                    DoSort();
                    handled = true;
                    break;
...
static void SetSortType
    (
    SortType sortType
    )
{
    if ( Prefs()->sortType == sortType ) {
        if ( Prefs()->sortOrder == SORTORDER_ASC )
...
static void DoIndexUpdate( void )
{
    ShowSortMessage();
    UpdateIndexList();
    HideMessage();
