/*
 * $Id: externalform.c,v 1.48 2007/02/15 15:16:43 prussar Exp $
 *
    ...
    switch ( event->eType ) {
        ...
        case keyDownEvent:
            if ( event->data.ctlEnter.controlID == frmExternalLinksCopy ||
                ...
                id = DmFindDatabase( 0, "Blazer" );

                if ( id != NULL ) {
                    Char* urlCopy;
                    Char* url;

                    url = FldGetTextPtr( field );
                    urlCopy = MemPtrNew( StrLen( url ) + 1 );

                    if ( urlCopy != NULL ) {
                        MemPtrSetOwner( urlCopy, 0 );
                        StrCopy( urlCopy, url );
                        if ( errNone != SysUIAppSwitch( 0, id, sysAppLaunchCmdGoToURL,
                                            urlCopy ) ) {
                            MemPtrFree( urlCopy );
                        }
                    }
