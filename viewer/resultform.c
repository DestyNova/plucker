/*
 * $Id: resultform.c,v 1.57 2006/11/12 02:00:56 prussar Exp $
 *
...
static void ReleaseSearchResult( void ) RESULTFORM_SECTION;
...
#endif            
static SearchResultType* SearchResult = NULL;
...
{
    MemSet( SearchResult, maxResult * sizeof *SearchResult, 0 );
    count           = 0;
    newLine         = false;
    ...
    )
{
    ...
    deltaY = lineHeight / 2;
    HiResAdjust( &deltaY, sonyHiRes | handeraHiRes );
...
/* Release allocated buffer used for search results */
static void ReleaseSearchResult( void )
{
    SafeMemPtrFree( SearchResult );
    SearchResult = NULL;
}
...
static void ResultFormInit( void )
{
    ...
    SearchResult = SafeMemPtrNew( maxResult * sizeof *SearchResult );
        ...
        case penUpEvent: {
            ...
            if ( lineMatch != NOT_FOUND ) {
                ...
                if ( RctPtInRectangle( x, y,
                    ...
                    match.data.frmGoto.matchCustom      = GOTO_SEARCH;
