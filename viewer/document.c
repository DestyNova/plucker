/*
 * $Id: document.c,v 1.183 2007/02/15 15:16:43 prussar Exp $
 *
...
static void ClearMetaRecord( void ) DOCUMENT_SECTION;
...
/* This should be called before changing what metaRecord points to. */
static void ClearMetaRecord( void )
{
    if ( metaRecord != NULL ) {
        CloseRecord( metaRecord, true );
        metaRecord = NULL;
    }
}
...
Int16 ParagraphNumToByteOffset
    (
    Int16  paragraphNum
    )
{
    ...
    offset   = GetParagraphOffset( record,
                   GET_PARAGRAPH( record, paragraphNum ) );
    ...
    )
{
    ...
    if ( Prefs()->toolbar == TOOLBAR_SILK ) {
        ...
        LstSetSelection( GetObjectPtr( frmMainPercentList ), percent / 10 );
        PalmSetCoordinateSystem( prevCoordSys );
    }
}
    ...
    )
{
    ...
    handle = FindRecord( recordId, &recordNum );
    if ( handle == NULL )
        return NO_RECORD;
    else
        FreeRecordHandle( &handle );
    ...
    )
{
    ...
    handle = FindRecord( GetCurrentRecordId(), &recordNum );
    if ( handle == NULL )
        return NO_RECORD;
    else
        FreeRecordHandle( &handle );
...
static void EstimateHeights( void )
{
    ...
    do {
         ...
         if ( handle == NULL ) {
            ...
            CloseRecord( metaRecordHandle, false );
        }
    } while ( ! done );
            ...
            )
{
    ...
    handle = FindRecord( lastRecordId, &recordNum );
    if ( handle == 0 )
        return lastRecordId;
    else
        FreeRecordHandle( &handle );
