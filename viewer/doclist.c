/*
 * $Id: doclist.c,v 1.38 2008/01/20 22:21:33 prussar Exp $
 *
    ...
    if ( plkrDocList != NULL ) {
        ...
        if ( version != PlkrDocListVersion ) {
            ...
            for ( index = 0; index < DmNumRecords( plkrDocList ); index++ ) {
                ...
                handle  = ResizeRecord( plkrDocList, index, newSize );
                if ( handle == NULL ) {
                    CloseDatabase( plkrDocList );
                    DmDeleteDatabase( cardNo, dbID );
                    plkrDocList = NULL;
                ...
                if ( writeTimestamp ) {
                    ...
                    MakeMetaName( metaName, META_PREFIX, dataPtr->location,
                        dataPtr->name );
    ...
    )
{
    ...
    for ( i = 0; i < DmNumRecords( plkrDocList ); i++ ) {
        ...
        handle = OpenRecord( plkrDocList, i );
        if ( handle == NULL ) {
            break;
        }
        handlePtr = MemHandleLock( handle );
