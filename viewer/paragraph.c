/*
 * $Id: paragraph.c,v 1.166 2006/11/12 02:00:55 prussar Exp $
 *
...
static Int16 GetParagraphHeight( void ) PARAGRAPH_SECTION;
static void InitializeParagraphContext( ParagraphContext* pContext,
                Paragraph* paragraph, Header* record )
                PARAGRAPH_SECTION;
...
static void WriteLine( ParagraphContext* pContext, TextContext* tContext,
                Char* until, Boolean skipLeadingSpace )
                PARAGRAPH_SECTION;
...
static void PushFunction( UInt8 code, FunctionHandler action )
    PARAGRAPH_SECTION;
#ifdef SUPPORT_ANNOTATION
static void InitializeAnnotations( UInt16 uid, UInt16 paragraphNum )
    PARAGRAPH_SECTION;
static void UpdateNextAnnotation( Char* position ) PARAGRAPH_SECTION;
#endif
static ParagraphFunction* PopFunction() PARAGRAPH_SECTION;
static FunctionType DoAnchorBegin( ParagraphContext* pContext,
                        TextContext* tContext, Int16* width ) PARAGRAPH_SECTION;
static FunctionType DoGeneralAnchorBegin( ParagraphContext* pContext,
                        TextContext* tContext, AnchorMode type )
                        PARAGRAPH_SECTION;
...
static void ForceDefaultColor( TextContext* tContext, AnchorMode type )
                        PARAGRAPH_SECTION;
static Boolean IsSoftHyphen( WChar token ) PARAGRAPH_SECTION;
#ifdef HAVE_RTL
static void RevStr( Char* str, Int16 len ) PARAGRAPH_SECTION;
#endif
...
static Int16         selectedWordParagraph;
static UInt16        selectedWordOffsets[ MAX_WORD_LENGTH ];
static UInt16*       currentTextOffsets;
#endif
...
static MemHandle   metaHandle;
static MetaRecord* meta;

static Int16 nextTargetByteOffset = 0;

#ifdef SUPPORT_WORD_LOOKUP
#define NEED_PARAGRAPHTOP
#endif
#ifdef SUPPORT_ANNOTATION
 #ifndef NEED_PARAGRAPHTOP
 #define NEED_PARAGRAPHTOP
 #endif
#endif

#ifdef NEED_PARAGRAPHTOP
static Char* paragraphTop;
#endif

#ifdef SUPPORT_ANNOTATION
static AnnotationEntry  nextAnnotation;
static Boolean          haveNextAnnotation = false;
static Char*            annotationStart;
static Char*            annotationStop;



AnnotationEntry* GetCurrentAnnotationHeader( void )
{
    if ( haveNextAnnotation )
...
static void InitializeAnnotations
    (
    UInt16 uid,
    UInt16 paragraphNum
    )
{
    haveNextAnnotation = GetAnnotationHeader( &nextAnnotation, uid,
                             paragraphNum, 0 );

    if ( haveNextAnnotation ) {
        annotationStart = paragraphTop + nextAnnotation.triggerStart;
        annotationStop  = paragraphTop + nextAnnotation.triggerStop;

        if ( nextAnnotation.flags & ANNOTATION_HIDDEN ||
             annotationStop <= annotationStart ) {
            UpdateNextAnnotation( 0 );
        }
    }
}
...
static void UpdateNextAnnotation
    (
    Char*   position
    )
{
    while ( haveNextAnnotation &&
            ( annotationStop <= position ||
              ( nextAnnotation.flags & ANNOTATION_HIDDEN ) ||
              annotationStop <= annotationStart ) ) {

        haveNextAnnotation = GetAnnotationHeader( &nextAnnotation,
                                 nextAnnotation.id.uid,
                                 nextAnnotation.id.paragraphNum,
                                 nextAnnotation.id.indexInParagraph + 1 );
     ...
     )
{
    ...
    if ( yPos <= tContext->findPixelY &&
        ...
        while ( offset < len ) {
            WChar ch;
            UInt16 offsetInParagraph;

            offsetInParagraph = currentTextOffsets[ offset ];
            ...
            if ( selectedWordLength < MAX_WORD_LENGTH ) {
                ...
                selectedWordOffsets[ selectedWordLength ] =
                    offsetInParagraph;
                selectedWordBuffer[ selectedWordLength++ ]          = ch;
                selectedWordParagraph = paragraphNum;
            }
...
/* Trim off any punctuation.  Note that the character
   ...
   The start and end positions returned are byte offsets relative to
   the paragraph top. */
    ...
    )
{
    ...
    if ( startPtr != NULL ) {
        *startPtr = selectedWordOffsets[ start ];
    }
    if ( endPtr != NULL ) {
        *endPtr = selectedWordOffsets[ i - 1 ] +
                      TxtGlueCharSize( selectedWord[ i - 1 ] );
    }
    if ( paragraphPtr != NULL ) {
        *paragraphPtr = selectedWordParagraph;
    }
    return selectedWord;
}
#endif
       ...
       )
{
    if ( tContext != NULL && Prefs()->forceDefaultColors &&
        ...
        tContext->foreColor.b   = ( anchor == ANCHORMODE_LINK ) ? 255 : 0;
...
/* Find paragraph number by text offset. */
UInt16 GetParagraphNumber
    (
    Header*     record,     /* pointer to record */
    UInt16      offset      /* offset of text */
    )
{
    Paragraph*  p;
    Paragraph*  p0;
    ...
    while ( pOffset <= offset && ( p - p0 ) < record->paragraphs ) {
        pOffset += p->size;
        p++;
    }
    ...
    )
{
    ...
    else if ( pContext->type == ALIGNMENT_JUSTIFY &&
...
{
    if ( ANCHORMODE_NONE != multiline.Anchor ) {
        Int16 tempX;
        ...
        tContext->activeAnchor = multiline.Anchor;
        tContext->cursorX      = tempX;

        ForceDefaultColor( tContext, multiline.Anchor );
    }
}
...
{
    if ( multiline.Anchor != ANCHORMODE_NONE )
...
static Int16 GetParagraphHeight( void )
{
    Int16        height;
    ...
    )
{
    if ( record->type == DATATYPE_PHTML_COMPRESSED )
        pContext->recordTop = MemHandleLock( GetUncompressTextHandle() );
    else
        pContext->recordTop = GET_DATA( record );

    pContext->position      = pContext->recordTop +
                                  GetParagraphOffset( record, paragraph );
    paragraphNum = paragraph - GET_PARAGRAPH( record, 0 );
...
/* Handle anchor of general type */
static FunctionType DoGeneralAnchorBegin
    (
    ParagraphContext*   pContext,
    TextContext*        tContext,
    AnchorMode          mode
    )
{
    if ( tContext != NULL ) {
        ...
        tContext->activeAnchor = mode;
        multiline.Anchor       = mode;
        
        reference = 0;

        if ( mode == ANCHORMODE_LINK ) {
            functionArgs = pContext->function + 1;
            reference    = *functionArgs * 256 + *( functionArgs + 1 );
        }
#ifdef SUPPORT_ANNOTATION
        else if ( mode == ANCHORMODE_ANNOTATION ) {
            reference = 0;
        }
#endif

        AnchorStart( tContext, reference, TOP_OFFSET, mode );
        ForceDefaultColor( tContext, mode );
    }
    return ANCHOR;
}
...
{
    return DoGeneralAnchorBegin( pContext, tContext, ANCHORMODE_LINK );
}
    ...
    )
{
    if ( tContext != NULL ) {
        ...
        tContext->activeAnchor  = ANCHORMODE_LINK;
        multiline.Anchor        = ANCHORMODE_LINK;

        functionArgs    = pContext->function + 1;
        reference       = *functionArgs * 256 + *( functionArgs + 1 );
        paragraphOffset = *( functionArgs + 2 ) * 256 + *( functionArgs + 3 );

        AnchorStart( tContext, reference, paragraphOffset, ANCHORMODE_LINK );
        AnchorSetByteOffset( tContext, nextTargetByteOffset );
        ForceDefaultColor( tContext, ANCHORMODE_LINK );
    }
    return ANCHOR;
}
...
{
    if ( tContext != NULL && tContext->activeAnchor != ANCHORMODE_NONE ) {
        tContext->activeAnchor  = ANCHORMODE_NONE;
        multiline.Anchor        = ANCHORMODE_NONE;
        AnchorStop( tContext, pContext->fontHeight );
        ForceDefaultColor( tContext, ANCHORMODE_NONE );
    }
    return ANCHOR;
}
    ...
    )
{
    ...
    if ( tContext == NULL ) {
    ...
    else if ( tContext->activeAnchor != ANCHORMODE_NONE ) {
        ...
        AnchorStart( tContext, alternateImage, TOP_OFFSET, ANCHORMODE_LINK );
        DrawInlineImage( inlinedImage, tContext, width );
        AnchorStopImage( tContext, pContext->fontHeight, *width );
        AnchorStop( tContext, pContext->fontHeight );
    }
    ...
    )
{
    ...
    Int16     len;
#ifdef SUPPORT_WORD_LOOKUP
    UInt16    offsets[ TEXT_BUFFER_SIZE + 1 ];
...
#ifdef SUPPORT_ANNOTATION
        if ( haveNextAnnotation ) {
            if ( annotationStop <= prevPosition ) {
                UInt16 ignore;

                if ( 0 < len ) {
#ifdef SUPPORT_WORD_LOOKUP
                    currentTextOffsets = offsets;
#endif
                    DrawText( chars, len, tContext );
                    tContext->cursorX += FntCharsWidth( chars, len );
                    len = 0;
                }
                DoAnchorEnd( pContext, tContext, &ignore );
                UpdateNextAnnotation( prevPosition + 1 );
            }
            if ( prevPosition == annotationStart ) {
                if ( 0 < len ) {
#ifdef SUPPORT_WORD_LOOKUP
                    currentTextOffsets = offsets;
#endif
                ...
                DoGeneralAnchorBegin( pContext, tContext, ANCHORMODE_ANNOTATION );
            }
        }
#endif
...
#ifdef SUPPORT_ANNOTATION
    if ( haveNextAnnotation && annotationStop <= until ) {
        UInt16 ignore;

        DoAnchorEnd( pContext, tContext, &ignore );
        UpdateNextAnnotation( until );
    }
#endif
...
#ifdef NEED_PARAGRAPHTOP
    paragraphTop = pContext.position;
#endif
...
/* FIXME: to remove this we must be able to assign a meta record
   for the test version or rewrite the code... */
...
#endif
#else
    metaHandle      = GetMetaRecord();
    meta            = MemHandleLock( metaHandle );
    paragraphHeight = GetParagraphHeight();
#ifdef SUPPORT_ANNOTATION
    InitializeAnnotations( meta->uid, paragraphNum );
#endif
#endif
    ...
    for ( ;; ) {
        ...
        {
            WriteLine( &pContext, tContext, until, true );
        }
    ...
    )
{
    if ( tContext != NULL ) {
        ...
        AnchorStart( tContext, reference, TOP_OFFSET, ANCHORMODE_LINK );
        if ( InsideCopyRegion( tContext ) )
