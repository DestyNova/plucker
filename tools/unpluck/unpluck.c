 ...
 * $Id: unpluck.c,v 1.13 2009/04/04 14:57:11 prussar Exp $
 *
...
int plkr_GetNextSequentialRecord
    (
    plkr_Document*  doc,
    int             record_index
    )
{
    ...
    cur = FindRecordByIndex(doc, record_index);
        ...
        if ((record->type == PLKR_DRTYPE_TEXT_COMPRESSED) ||
            ...
            if (!handle->seek (handle, record->offset) ||
                (handle->read (handle, buf, record->size, record->size) !=
                 record->size)) {
                _plkr_message
                    ("Bad read from compressed DBHandle while reading record %d",
                     record->uid);
                free (buf);
                if (tbuffer != buffer)
                    free (tbuffer);
            ...
            if (!handle->seek (handle, record->offset) ||
                (handle->read (handle, tbuffer, blen, size_needed) !=
                 size_needed)) {
                _plkr_message
                    ("Bad read from uncompressed DBHandle while reading record %d",
                     record->uid);
                if (tbuffer != buffer)
                    free (tbuffer);
                return FALSE;
    ...
    for (i = 0; i < nreserved; i++) {
        if (reserved[i].name == PLKR_HOME_NAME)
            ...
            if (!ParseURLs (newdoc, handle)) {
                ...
                //FreePluckerDoc (newdoc);
                //return NULL;
                newdoc->urls_index_record_uid = 0;
            }
        }
    }
    ...
    )
{
    ...
    toread  = MIN (buffersize, readsize);

    result = read ((int) (handle->dbprivate), buffer, toread);

    if (result != toread) {
          _plkr_message
              ("Unable to read %d bytes from fp %d -- read %d instead\n",
               toread, (int) (handle->dbprivate), result);
    }
...
unsigned char *plkr_GetRecordBytes
    ...
    if (!GetUncompressedRecord (doc, doc->handle, record_index,
        ...
        *type = record->type;
        *continued = record->flags & PLKR_FLAG_CONTINUED;
        return buf;
    }
}
