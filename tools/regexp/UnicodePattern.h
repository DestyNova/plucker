#ifndef __UNICODEPATTERN_H__
#define __UNICODEPATTERN_H__
...
/**
  ...
  java.util.regex.UnicodePattern class. The pattern class represents an immutable
  ...
  split apart. The {@link UnicodeMatcher UnicodeMatcher} class represents the maching
  object.

  The UnicodePattern class works primarily off of "compiled" patterns. A typical
  instantiation of a regular expression looks like:

  <pre>
  UnicodePattern * p = UnicodePattern::compile(L"a*b");
  UnicodeMatcher * m = p->createUnicodeMatcher(L"aaaaaab");
  if (m->matches()) ...
  </pre>

  However, if you do not need to use a pattern more than once, it is often times
  okay to use the UnicodePattern's static methods insteads. An example looks like this:

  <pre>
  if (UnicodePattern::matches(L"a*b", L"aaaab")) { ... }
  </pre>

  This class does not currently support unicode. The unicode update for this
  class is coming soon.

  This class is partially immutable. It is completely safe to call createUnicodeMatcher
  concurrently in different threads, but the other functions (e.g. split) should
  not be called concurrently on the same <code>UnicodePattern</code>.
  ...
  <hr>
   ...
   <i>Begin Text Extracted And Modified From java.util.regex.UnicodePattern documentation</i>
   ...
   <p>The <code>UnicodePattern</code> engine performs traditional NFA-based matching
   ...
   <ul>
      ...
      by the <CODE>UnicodeMatcher</CODE> class: Repeated invocations of the
  ...
  <i>End Text Extracted And Modified From java.util.regex.UnicodePattern documentation</i>
...
class UnicodePattern
{
  ...
  protected:
     ...
     */
    static std::map<std::wstring, std::pair<std::wstring, unsigned long> > registeredUnicodePatterns;
  protected:
    /**
  ...
  protected:
     ...
     */
    NFAUNode * parseRegisteredUnicodePattern(NFAUNode ** end);
    /**
  ...
  public:
    /**
      ...
      <code>UnicodePattern</code> object. Special values can be assigned to
      <code>mode</code> when certain non-standard behaviors are expected from
      the <code>UnicodePattern</code> object.
      @param pattern The regular expression to compile
      @param mode    A bitwise or of flags signalling what special behaviors are
                     wanted from this <code>UnicodePattern</code> object
      @return If successful, <code>compile</code> returns a <code>UnicodePattern</code>
              pointer. Upon failure, <code>compile</code> returns
              <code>NULL</code>
     */
    static UnicodePattern                    * compile        (const std::wstring & pattern,
    ...
    /**
      ...
      @return If successful, <code>compileAndKeep</code> returns a
              <code>UnicodePattern</code> pointer. Upon failure, <code>compile</code>
              returns <code>NULL</code>.
     */
    static UnicodePattern                    * compileAndKeep (const std::wstring & pattern,
                                                        const unsigned long mode = 0);
    ...
    /**
      ...
      <p>
      <code>
      UnicodePattern::replace(L"(a+)b(c+)", L"abcccbbabcbabc", L"\\2b\\1");
      </code>
      <p>
      which would replace <code>abcccbbabcbabc</code> with
      <code>cccbabbcbabcba</code>.
      @param pattern  The regular expression
      @param replace  The string in which to perform replacements
      @param str      The replacement text
      @param mode     The special mode requested of the <code>UnicodePattern</code>
    ...
    /**
      ...
      fclose(fp);<br>
      <br>
      std::vector&lt;std::wstring&gt; lines = UnicodePattern::split(L"[\r\n]+", str, true);<br>
      <br>
      </code>
    ...
    /**
      ...
      <p>
      <code>
      std::vector&lt;td::string&gt; numbers = UnicodePattern::findAll(L"\\d+", string);
      </code>
      <p>
    ...
    /**
      ...
      <p>
      <code>
      UnicodePattern::registerUnicodePattern(L"ip", L"(?:\\d{1,3}\\.){3}\\d{1,3}");<br>
      UnicodePattern * p1 = UnicodePattern::compile(L"{ip}:\\d+");<br>
      UnicodePattern * p2 = UnicodePattern::compile(L"Connection from ({ip}) on port \\d+");<br>
      </code>
      <p>
      Multiple calls to <code>registerUnicodePattern</code> with the same
      ...
      @return Success/Failure. Fails only if <code>pattern</code> has invalid
              syntax
     */
    static bool                         registerUnicodePattern(const std::wstring & name,
    ...
    /**
      ...
      */
    static void                         unregisterUnicodePatterns();
    /**
      Don't use
     */
    static void                         clearUnicodePatternCache();
    ...
    /**
      ...
      <p>
      <code>
      std::pair&lt;std::wstring, int&gt; match = UnicodePattern::findNthMatch(L"\\d{1,3}", L"192.168.1.101:22", 1);<br>
  ...
  public:
     ...
     */
    std::wstring               getUnicodePattern     () const;
    /**
      ...
      @return A new matcher using object using this pattern and the specified
              string
     */
    UnicodeMatcher                 * createUnicodeMatcher  (const std::wstring & str);
};
...
class NFAUNode
{
  ...
  public:
    ...
    virtual int match(const std::wstring & str, UnicodeMatcher * matcher, const int curInd = 0) const = 0;
};
class NFACharUNode : public NFAUNode
{
  ...
  public:
    ...
    virtual int match(const std::wstring & str, UnicodeMatcher * matcher, const int curInd = 0) const;
};
...
class NFAQuantifierUNode : public NFAUNode
{
  public:
    ...
    NFAUNode * inner;
    virtual void findAllNodes(std::map<NFAUNode*, bool> & soFar);
    NFAQuantifierUNode(UnicodePattern * pat, NFAUNode * internal,
                      const int minMatch = UnicodePattern::MIN_QMATCH,
                      const int maxMatch = UnicodePattern::MAX_QMATCH);
    virtual int match(const std::wstring & str, UnicodeMatcher * matcher, const int curInd = 0) const;
};
class NFAGreedyQuantifierUNode : public NFAQuantifierUNode
{
  public:
    NFAGreedyQuantifierUNode(UnicodePattern * pat, NFAUNode * internal,
                            const int minMatch = UnicodePattern::MIN_QMATCH,
                            const int maxMatch = UnicodePattern::MAX_QMATCH);
    virtual int match(const std::wstring & str, UnicodeMatcher * matcher, const int curInd = 0) const;
    virtual int matchInternal(const std::wstring & str, UnicodeMatcher * matcher, const int curInd, const int soFar) const;
};
class NFALazyQuantifierUNode : public NFAQuantifierUNode
{
  public:
    NFALazyQuantifierUNode(UnicodePattern * pat, NFAUNode * internal,
                          const int minMatch = UnicodePattern::MIN_QMATCH,
                          const int maxMatch = UnicodePattern::MAX_QMATCH);
    virtual int match(const std::wstring & str, UnicodeMatcher * matcher, const int curInd = 0) const;
};
...
{
  public:
    NFAPossessiveQuantifierUNode(UnicodePattern * pat, NFAUNode * internal,
                                const int minMatch = UnicodePattern::MIN_QMATCH,
                                const int maxMatch = UnicodePattern::MAX_QMATCH);
    virtual int match(const std::wstring & str, UnicodeMatcher * matcher, const int curInd = 0) const;
};
...
class NFAGroupLoopUNode : public NFAUNode
{
  public:
    ...
    virtual void findAllNodes(std::map<NFAUNode*, bool> & soFar);
    virtual int match(const std::wstring & str, UnicodeMatcher * matcher, const int curInd = 0) const;
    int matchGreedy(const std::wstring & str, UnicodeMatcher * matcher, const int curInd = 0) const;
    int matchLazy(const std::wstring & str, UnicodeMatcher * matcher, const int curInd = 0) const;
    int matchPossessive(const std::wstring & str, UnicodeMatcher * matcher, const int curInd = 0) const;
};
