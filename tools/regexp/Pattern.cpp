/**
  ...
  bug fixes. The header files for Pattern and Matcher are fairly well documented
...
inline static int tolower(const char c)
{
...
std::string Pattern::parsePosix()
{
  ...
  if (s7 == "{Lower}")  return "abcdefghijklmnopqrstuvwxyz";
  if (s7 == "{Upper}")  return "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
  if (s7 == "{Alpha}")  return "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
  if (s7 == "{Digit}")  return "0123456789";
  if (s7 == "{Alnum}")  return "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
  if (s7 == "{Punct}")  return "!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~";
  if (s7 == "{Graph}")  return "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~";
  if (s7 == "{Print}")  return "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~";
  if (s7 == "{Blank}")  return " \t";
  if (s7 == "{Space}")  return " \t\n\x0B\f\r";
  if (s7 == "{Cntrl}")
  {
  ...
  if (pattern.substr(curInd, 8) == "{XDigit}") return "abcdefABCDEF0123456789";
  raiseError();
  return "";
}
NFANode * Pattern::parseBackref()
{
  ...
  curInd = ci - 1;
  return registerNode(new NFAReferenceNode(oldRef));
  ...
  while (curInd < (int)pattern.size())
  {
    ...
    case '\\':
      if      (curInd < (int)pattern.size())
      {
        ...
        case 'A': ++curInd; next = new NFAStartOfInputNode;     break;
        case 'B': ++curInd; next = new NFAWordBoundaryNode(0);  break;
        case 'b': ++curInd; next = new NFAWordBoundaryNode(1);  break;
        case 'G': ++curInd; next = new NFAEndOfMatchNode;       break;
        case 'Z': eoi = 1;
        case 'z': ++curInd; next = new NFAEndOfInputNode(eoi);  break;
        default:
          t = parseEscape(inv, quo);
    ...
    case '(':
      {
        ...
        t1 = parse(1, 0, &end);
        if (!t1) raiseError();
        else if (dynamic_cast<NFAGroupHeadNode*>(t1) != NULL && (t2 = quantifyGroup(t1, end, grc)) != NULL)
        {
          cur->next = t2;
          cur = t2->next;
        }
...
Pattern * Pattern::compile(const std::string & pattern, const unsigned long mode)
{
  ...
  else
  {
    ...
    {
      if (dynamic_cast<NFAStartOfInputNode*>(p->head) == NULL)
      {
...
std::string Pattern::replace(const std::string & pattern, const std::string & replace,
                        const std::string & str, const unsigned long mode)
{
  std::string ret;
...
bool Pattern::registerPattern(const std::string & name, const std::string & pattern, const unsigned long mode)
{
  Pattern * p = Pattern::compile(pattern, mode);
  if (!p) return 0;
  Pattern::registeredPatterns[name] = make_pair(pattern, mode);
  delete p;
  return 1;
}
...
Pattern::~Pattern()
{
  ...
  */
  ...
  for (std::map<NFANode*, bool>::iterator it = nodes.begin(); it != nodes.end(); ++it) delete it->first;
}
std::string Pattern::replace(const std::string & str, const std::string & replace)
{
  int li = 0;
  std::string ret = "";
